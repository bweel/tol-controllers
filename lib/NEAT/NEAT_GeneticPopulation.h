...
namespace NEAT
{
    ...
    {
        vector<boost::shared_ptr<GeneticGeneration> > generations;

        vector<boost::shared_ptr<GeneticSpecies> > species;

        //we use a separate vector for extinct species to save CPU.
        vector<boost::shared_ptr<GeneticSpecies> > extinctSpecies;
    ...
    public:
        ...
        NEAT_DLL_EXPORT vector<boost::shared_ptr<GeneticIndividual> >::iterator getIndividualIterator(int a,int generation=-1);
